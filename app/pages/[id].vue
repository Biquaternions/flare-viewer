...
const { data, error } = useFetch<string>(`${config.public.apiBackendUrl}/api/v1/flareprofiler/${id}`, {
  method: "get",
})
...
onMounted(() => {
  watchEffect(async () => {
    ...
    if (data.value) {
      ...
      dataSource.value = new EventSource(`${config.public.apiBackendUrl}/api/v1/flarestream/data/${id}`)
      ...
      timelineSource.value = new EventSource(`${config.public.apiBackendUrl}/api/v1/flarestream/timeline/${id}`)
...
useSeoMeta({
  ...
  ogImage: new URL(`/api/v1/flarethumbnail/${id}.png`, appConfig.url).toString(),

  twitterTitle: `${appConfig.title} - ${id}`,
  twitterDescription: appConfig.description,
  twitterImage: new URL(`/api/v1/flarethumbnail/${id}.png`, appConfig.url).toString(),
